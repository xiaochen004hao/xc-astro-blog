...
tags: ["测试", "markdown"]
pinned: true
aiSummary: 这篇文章是Astro主题的Markdown语法全面展示页，系统演示了该主题支持的所有Markdown元素用法。内容从标题层级、水平分割线、文本强调和删除线等基础语法讲起，逐步深入到引用块嵌套、参考文献脚注等进阶功能，同时涵盖无序列表与有序列表的创建方式。文章还重点展示了代码相关的多种写法，包括内联代码、缩进代码块和围栏代码块，并借助ExpressiveCode插件实现了JavaScript语法高亮、代码行标记和文件标题等增强特性，最后补充了表格对齐、键盘快捷键和图片链接等常用元素的使用示例。
